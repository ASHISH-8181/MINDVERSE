...
const ConnectToSeniors = () => {
  ...
  useEffect(() => {
    // Load seniors from localStorage
    try {
      ...
      console.error('Error loading senior profiles from localStorage:', error);
  ...
  const filteredSeniors = seniors.filter((profile) => {
    ...
    const matchesMarks =
      (filterMinMarks === '' ||
        profile.subjects.some((s) => parseInt(s.marks) >= parseInt(filterMinMarks))) &&
      (filterMaxMarks === '' ||
        profile.subjects.some((s) => parseInt(s.marks) <= parseInt(filterMaxMarks)));
  ...
  const handleReviewSubmitted = () => {
    // Reload senior profiles from localStorage
    try {
      const storedSeniors = localStorage.getItem('seniors');
      if (storedSeniors) {
        setSeniors(JSON.parse(storedSeniors));
      ...
      console.error('Error loading senior profiles after review:', error);
        ...
        {filteredSeniors.map((profile) => (
          <div key={profile.id} className="bg-white dark:bg-blue-900/50 rounded-lg p-6 flex items-center space-x-6 shadow-lg border border-gray-200 dark:border-blue-700">
            ...
            <div className="flex-1">
              <h2 className="text-xl font-semibold mb-1 text-gray-900 dark:text-white">{profile.name}</h2>
              {/* Displaying multiple subjects and their marks */}
              <div className="text-blue-600 dark:text-blue-400 text-sm mb-1">
                {profile.subjects && profile.subjects.map((subjectData, index) => (
                  <p key={index}>
                    {subjectData.subject}: {subjectData.marks}
                  </p>
                ))}
              </div>
              <p className="text-gray-700 dark:text-gray-300 text-sm mb-1"><strong>Description:</strong> {profile.description}</p>
